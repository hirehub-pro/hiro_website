import { slugifyProfession } from './search-routing';
...
function getLocalizedBrand(locale) {
  if (locale === 'he') return 'הירו';
  if (locale === 'ar') return 'هيرو';
  return 'Hiro';
}

function getLocalizedProfessionTitle(professionName, locale) {
  const brand = getLocalizedBrand(locale);
  const name = String(professionName || '').trim();

  if (locale === 'he') {
    return `${brand} | ${name} מומלצים באזור שלכם`;
  }

  if (locale === 'ar') {
    return `${brand} | ${name} موصى بهم في منطقتك`;
  }

  return `${brand} | Recommended ${name} in your area`;
}

function getLocalizedProfessionDescription(professionName, locale) {
  const name = String(professionName || '').trim();

  if (locale === 'he') {
    return `מחפשים ${name} לידכם? בהירו תוכלו למצוא בעלי מקצוע מומלצים באזור, לבדוק ביקורות וליצור קשר במהירות.`;
  }

  if (locale === 'ar') {
    return `تبحثون عن ${name} بالقرب منكم؟ يساعدكم هيرو على العثور على مختصين موصى بهم، مراجعة التقييمات والتواصل بسرعة.`;
  }

  return `Looking for ${name} near you? Hiro helps you find recommended local professionals, check reviews, and contact them quickly.`;
}

export function findProfessionMetadataBySlug(items, slug) {
  const normalizedSlug = String(slug || '').trim().toLowerCase();
  const professionItems = Array.isArray(items) ? items : [];

  return professionItems.find((item) => {
    const source = item?.en || item?.logo || item?.he || item?.ar;
    return slugifyProfession(source) === normalizedSlug;
  }) || null;
}

function getLocalizedFallbackSeo(readableName, locale) {
  return {
    title: getLocalizedProfessionTitle(readableName, locale),
    description: getLocalizedProfessionDescription(readableName, locale),
    intro: getLocalizedProfessionDescription(readableName, locale),
...
export function getProfessionSeoData(slug, locale = 'en', displayName = '') {
  const normalizedSlug = String(slug || '').trim().toLowerCase();
  const professionData = PROFESSION_SEO_MAP[normalizedSlug];
  const selectedLocale = ['en', 'he', 'ar'].includes(locale) ? locale : 'en';

  if (!professionData) {
    const readableName = String(displayName || '').trim() || humanizeSlug(normalizedSlug);
    return getLocalizedFallbackSeo(readableName, selectedLocale);
  }

  const localizedData = professionData[selectedLocale] || professionData.en;
  const professionName = String(
    displayName
      || (selectedLocale === 'en' ? humanizeSlug(normalizedSlug) : localizedData.keywords?.[0])
      || localizedData.title
  ...
  return {
    title: getLocalizedProfessionTitle(professionName, selectedLocale),
    description: getLocalizedProfessionDescription(professionName, selectedLocale),
