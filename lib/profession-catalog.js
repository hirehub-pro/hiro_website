...
function normalizeProfessionLookupValue(value) {
  return String(value ?? '').trim().toLocaleLowerCase();
}

const PROFESSION_LOOKUP = new Map();

for (const profession of PROFESSION_CATALOG) {
  const candidates = [
    profession.id,
    profession.slug,
    profession.value,
  ...
  for (const candidate of candidates) {
    const normalized = normalizeProfessionLookupValue(candidate);
    if (normalized && !PROFESSION_LOOKUP.has(normalized)) {
      PROFESSION_LOOKUP.set(normalized, profession);
    }
  }
}

export function findProfessionByValue(value) {
  if (value && typeof value === 'object') {
    ...
    for (const candidate of candidates) {
      const match = PROFESSION_LOOKUP.get(normalizeProfessionLookupValue(candidate));
      if (match) return match;
    }
  }

  return PROFESSION_LOOKUP.get(normalizeProfessionLookupValue(value)) || null;
}

export function getLocalizedProfessionValue(value, locale = 'he') {
  const profession = findProfessionByValue(value);
  if (profession) return getProfessionLabel(profession, locale);

  if (value && typeof value === 'object') {
    return String(value[locale] || value.en || value.he || value.ar || value.am || value.ru || '').trim();
  }
