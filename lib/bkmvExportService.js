...
async function buildPrintedSummaryPdf({ business, system, fromDate, toDate, exportedAt, documents }) {
  ...
  const html = `
    ...
    <section class="bkmv-print-page">
      <div class="bkmv-subtitle">
        <div class="bkmv-subtitle-row">
          <span class="bkmv-subtitle-label">מספר עוסק מורשה:</span>
          <span>${businessNumber}</span>
        </div>
        <div class="bkmv-subtitle-row">
          <span class="bkmv-subtitle-label">שם בית העסק:</span>
          <span>${businessName}</span>
        </div>
        <div class="bkmv-subtitle-row">
          <span class="bkmv-subtitle-label">טווח תאריכי הנתונים:</span>
          <span>${dateRange}</span>
        </div>
      </div>
...
function buildZip({ bkmvText, iniText, folderPath, printedSummaryName, printedSummaryBlob, annex4Name, annex4Blob }) {
  return buildZipBlob([
    ...
    { name: `${folderPath}/${printedSummaryName}`, content: printedSummaryBlob },
    { name: `${folderPath}/${annex4Name}`, content: annex4Blob },
  ]);
}
  ...
  async exportForUser({ user, fromDate, toDate }) {
    ...
    return {
      ...
      packages: [{
        ...
        printedSummaryPdf: `${folderPath}/${printedSummaryName}`,
        annex4Pdf: `${folderPath}/${annex4Name}`,
      }],
      zipFile: zipName,
      warnings: [],
      files: [
