...
export function buildD110({ business, document, item, index, recordNumber, linkId }) {
  ...
  const lineTotalBeforeTax = Number(item?.subtotal ?? item?.lineSubtotal ?? item?.totalAmount ?? item?.amount ?? (rawPrice * quantity)) || 0;
  const priceBeforeTax = quantity > 0 ? lineTotalBeforeTax / quantity : rawPrice;
