...
export function getProfessionPageContent(profession, locale = 'he') {
  ...
  const editorialDescription = editorial
    ? `מחפשים ${name}? מצאו בעלי מקצוע ל${featuredServices}. השוו ניסיון, אזורי שירות וביקורות ופנו ישירות לבעל המקצוע המתאים.`
    : copy.description(name);
  const editorialIntro = editorial
    ? `${name} יכולים לסייע ב${featuredServices} ובשירותים נוספים שמופיעים בהמשך. בדקו ניסיון רלוונטי, רישוי כשנדרש, עבודות קודמות וחוות דעת לפני שמחליטים.`
    : copy.intro(name);
