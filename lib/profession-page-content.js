...
const COPY = {
  en: {
    ...
    editorialDescription: (name, services) => `Find ${name} for ${services}. Compare relevant experience, service areas and reviews, then contact the right professional directly.`,
  ...
  he: {
    ...
    editorialDescription: (name, services) => `מחפשים ${name}? מצאו בעלי מקצוע ל${services}. השוו ניסיון, אזורי שירות וביקורות ופנו ישירות לבעל המקצוע המתאים.`,
  ...
  ar: {
    ...
    editorialDescription: (name, services) => `ابحثوا عن ${name} لخدمات ${services}. قارنوا الخبرة ومناطق الخدمة والتقييمات ثم تواصلوا مباشرة مع المختص المناسب.`,
  ...
  am: {
    ...
    editorialDescription: (name, services) => `ለ${services} የ${name} ባለሙያዎችን ያግኙ። ልምድን፣ የአገልግሎት አካባቢንና ግምገማዎችን ያወዳድሩ።`,
  ...
  ru: {
    ...
    editorialDescription: (name, services) => `Найдите специалистов «${name}» для услуг: ${services}. Сравните опыт, район работы и отзывы, затем свяжитесь с подходящим специалистом.`,
...
export function getProfessionPageContent(profession, locale = 'he') {
  ...
  const featuredServices = services.slice(0, 3).join(', ');
  const editorialDescription = editorial
    ? copy.editorialDescription(name, featuredServices)
