...
export function getProfilePageSeo(profile, { schedule = false, professionLabels = null } = {}) {
  ...
  const profileDescription = cleanText(profile?.description || '');
  ...
  const baseDescription = schedule
    ...
    : `${summaryParts.join(' ')}.`;
  const profileDescriptionWithContext = [
    profileDescription,
    baseDescription,
  ].filter(Boolean).join(' ');
  ...
  return {
    title,
    description: truncateText(profileDescriptionWithContext || `צפו בפרופיל של ${name} בהירו, קראו ביקורות, בדקו שירותים וזמינות ופנו ישירות.`),
  };
}
