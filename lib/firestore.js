...
export async function saveUserInvoice(uid, invoiceData) {
  ...
  const normalizedDocType = rawDocType === 'receipt' ? 'invoice' : rawDocType;
  const normalizedPaymentMethod = String(invoiceData?.payments?.[0]?.type || '').trim().toLowerCase() || 'cash';
  const normalizedItems = Array.isArray(invoiceData?.lineItems)
    ...
    : [];
  const normalizedPaymentMethods = Array.isArray(invoiceData?.payments) && invoiceData.payments.length > 0
  ...
  const docPayload = {
    ...
    discountAmount: 0,
    docType: normalizedDocType,
    fileName: String(invoiceData?.savedFileName || invoiceData?.fileName || `invoice_${uid}_${createdAtMs}.html`),
    hasDiscount,
    invoiceDocId,
    invoiceNumber: normalizedInvoiceNumber,
    items: normalizedItems,
    name: normalizedDocumentDescription || `Invoice #${normalizedInvoiceNumber}${normalizedClientName ? ` - ${normalizedClientName}` : ''}`,
    notes: String(invoiceData?.notes || '').trim(),
    paidAmount,
    paymentAmountTotal: Array.isArray(invoiceData?.payments)
  ...
  const docRef = await addDoc(collection(db, 'users', uid, 'invoices'), docPayload);
