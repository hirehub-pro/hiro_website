...
export async function saveUserVerificationInfo(uid, verificationData) {
  ...
  const normalizedData = {
    ...
    businessId: String(verificationData?.businessId || '').trim(),
...
export async function getUserSavedInvoices(uid) {
  ...
  const q = query(
    collection(db, 'users', uid, 'saved_invoices'),
...
export async function saveUserInvoice(uid, invoiceData) {
  ...
  const compactDate = issueDate ? issueDate.replaceAll('-', '') : '';
  const docType = String(invoiceData?.docType || 'receipt');
  ...
  const docPayload = {
    ...
    date: compactDate,
    docType,
    fileName: `invoice_${uid}_${createdAtMs}.pdf`,
    invoiceNumber: normalizedInvoiceNumber,
    name: normalizedDocumentDescription || `${getSavedInvoiceTypeLabel(docType)} ${normalizedInvoiceNumber}${normalizedClientName ? ` - ${normalizedClientName}` : ''}`,
    payload: invoiceData,
    storagePath: '',
    url: '',
  };

  const docRef = await addDoc(collection(db, 'users', uid, 'saved_invoices'), docPayload);
